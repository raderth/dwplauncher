...
class Downloader:
    ...
    def _fetch_manifest(self) -> tuple[list[dict], list[dict]]:
        ...
        url = f"{self.download_url}/manifest"
        log.info("GET %s", url)
            ...
            try:
                async with aiohttp.ClientSession(connector=connector) as session:
                    while not self._stop.is_set():
                        ...
                        url = f"{self.download_url}/chunk/{chunk_id}"
    ...
    def _repair_individual(self, entries: list[dict]) -> None:
        ...
        for idx, entry in enumerate(entries):
            ...
            url = f"{self.download_url}/file/{entry['hash']}"
    ...
    def _write_version(self):
        try:
            remote = version.server_version(self.download_url)
